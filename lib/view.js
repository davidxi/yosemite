...
APP.componentFailUpdate_ = function(html, error) {
...
function updateRouteURIonLink_(event) {
    var el = $(event.currentTarget);
    if (!el.is('a')) return;
    if (/^_top$/i.test(el.attr('target'))) return;
    if (el.attr('data-ignore-urirewrite')) return;
    if (el.data('yoseHasUpdateLinkURI')) return;
    el.attr('href', APP.facadeURI_(el.attr('href')));
    ...
    container.find('form').each(function(_, form) {
        ...
        $form.on('submit', function(event) {
            ...
            req.done(function(html) {
                container.empty();
                container.html(html);
                APP.pageletDidUpdate_(container, html, formSubmitParams);
            });
            return false;
        });
    });
