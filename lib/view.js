...
var invariant = _.invariant;

APP.updateTransport_ = function(container, html, xhrParams) {
    _.isElement(container) && (container = $(container));
    invariant(container instanceof jQuery);
    invariant($.type(html) === 'string');
    invariant(!xhrParams || $.isPlainObject(xhrParams));

    APP.componentWillUpdate_(container, html, xhrParams);
    container.empty();
    try {
        container.html(html);
    } catch (e) {
        APP.componentFailUpdate_(container, html, xhrParams, e);
        return false;
    }
    APP.componentDidUpdate_(container, html, xhrParams);
    return true;
};

APP.componentWillUpdate_ = function(container, html) {
    _.isElement(container) && (container = $(container));
    invariant(container instanceof jQuery);
    invariant($.type(html) === 'string');

    if (container[0] === APP.container_[0]) {
        // reset <frameset> structure flag
        APP.hasPagelet = false;
    }
    APP.componentWillUpdate(container, html, container.attr('data-pagelet-id'));
};

APP.componentDidUpdate_ = function(container, html, reqParams) {
    invariant(container instanceof jQuery);
    invariant($.type(html) === 'string');
    ...
    container.find('form').each(function(_, form) {
        ...
        attachFormSubmitInterceptor_(form, container);
    });
    ...
    APP.componentDidUpdate(container, html, container.attr('data-pagelet-id'));
};

APP.componentFailUpdate_ = function(container, html, xhrParams, error) {
    _.isElement(container) && (container = $(container));
    invariant(container instanceof jQuery);
    invariant($.type(html) === 'string');
    invariant(!xhrParams || $.isPlainObject(xhrParams));

    return APP.componentFailUpdate(container, html, error);
};

/**
 * interceptors
 */
function attachFormSubmitInterceptor_(form, componentContainer) {
    invariant(_.isElement(form) && (form.tagName.toUpperCase() === 'FORM'));
    invariant(componentContainer instanceof jQuery);
    ...
    $(form).on('submit', function(event) {
        ...
        return interceptFormSubmit_(event, form, componentContainer);
    });
}

function interceptFormSubmit_(event, form, componentContainer, xhrDoneCb) {
    invariant(event && event.target);
    invariant(_.isElement(form) && (form.tagName.toUpperCase() === 'FORM'));
    invariant(componentContainer instanceof jQuery);
    ...
    xhr.done(function(resp) {
        ...
        var ret = APP.updateTransport_(targetContainer, resp, formSubmitParams);
        if (!ret) return;
        APP.hasPagelet || APP.goDummyURI(formSubmitParams);
        APP.componentDidUpdate_(targetContainer, resp, formSubmitParams);
    });
    return false;
}
...
APP.updatePagelet = function(nodeId, url) {
    ...
    APP.container_.off('click');
    APP.container_.off('contextmenu');
    ...
    req.done(function(html) {
        APP.updateTransport_(container, html, params, nodeId);
    });
};
