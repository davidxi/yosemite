 ...
 */
Callbacks.componentWillRoute = function(reqParams) {
    $invariant(!reqParams || $.isPlainObject(reqParams));
    return $callbacksPublic.componentWillRoute(reqParams);
};
 ...
 */
Callbacks.componentWillUnmount = function(jumpUri) {
    $invariant($.type(jumpUri) === 'string');
    return $callbacksPublic.componentWillMount(jumpUri);
};
